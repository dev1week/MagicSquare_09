...
class MagicSquareJudge:
    ...
    def is_magic(self, grid: list[list[int]]) -> bool:
        ...
        if not self._diagonal_equal_constant(
            [grid[index][index] for index in range(GRID_SIZE)]
        ):
            return False

        if not self._diagonal_equal_constant(
            [grid[row_index][GRID_SIZE - 1 - row_index] for row_index in range(GRID_SIZE)]
        ):
            return False
    ...
    def _all_columns_equal_constant(self, grid: list[list[int]]) -> bool:
        for column_index in range(GRID_SIZE):
            column = [grid[row_index][column_index] for row_index in range(GRID_SIZE)]
